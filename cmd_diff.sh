...
function add_cmds_path()
{
	result_lines=`cat $RESULT_TXT | wc -l`
	readarray -t result_arr < $RESULT_TXT

	rm -rf $RESULT_TXT

	for ((idx=0; idx<result_lines; idx++))
	do
		echo -e "${result_arr[$idx]}\t\t\t\t `whereis ${result_arr[$idx]} | awk -F " " '{print $2}'`" >> $RESULT_TXT
	done
}
